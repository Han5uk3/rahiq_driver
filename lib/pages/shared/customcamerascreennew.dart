  ...
  Future<void> _stopRecording() async {
    ...
    try {
      ...
      try {
        ...
        final resolution = _controller!.value.previewSize;
        debugPrint('🎥 [CustomCamera] RAW Video (before compression):');
        debugPrint('🎥 [CustomCamera] - Path: ${videoFile.path}');
        debugPrint(
          '🎥 [CustomCamera] - Raw Size: ${(size / (1024 * 1024)).toStringAsFixed(2)} MB',
        );
        debugPrint('🎥 [CustomCamera] - Duration: ${_recordingSeconds}s');
        if (resolution != null) {
          debugPrint(
            '🎥 [CustomCamera] - Preview Resolution: ${resolution.width} x ${resolution.height}',
          );
        }
      } catch (logError) {
